---
title: Compare nonprofit debt-management counselors
description: A Maryland-focused, evidence-based shortlist, transparent ranking method, BBB context, and decision tree for credit-card debt help.
---
...
```mermaid
flowchart TD
  A[Need help with mostly unsecured credit-card debt] --> B{Summons, freeze, lien, foreclosure, SSI/Medicaid change, or material home equity?}
  B -->|Yes| C[Legal / benefits / housing review first]
  B -->|No| D{Monthly surplus after essentials and buffer?}
  D -->|Zero or negative| E[Do not enroll in a DMP; ask about hardship, legal options, and bankruptcy screening]
  D -->|Positive| F[Get free written screening from MMI and GreenPath]
  F --> G{Does either written plan cover the cards and fit the real budget?}
  G -->|No| H[Try InCharge or Apprisen; do not use home collateral to force fit]
  G -->|Yes| I[Compare total paid, payoff date, setup/monthly fees, cards closed, and missed-payment rules]
  I --> J[Enroll only after creditors confirm participation and every promise is written]
```
